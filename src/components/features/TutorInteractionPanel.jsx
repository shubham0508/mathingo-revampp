...
import { Button as UIButton } from '../ui/button'; // Aliasing to avoid conflict
...
const TutorInteractionPanel = ({
    ...
    const isButtonSelected = (buttonType) => {
        if (isLoading) {
            return currentButtonType === buttonType ||
                (buttonType === 'hints' && (currentButtonType === 'next_hint' || currentButtonType === 'next_3hint')) ||
                (buttonType === 'steps' && (currentButtonType === 'next_step' || currentButtonType === 'next_3step')) ||
                (buttonType === 'verify' && currentButtonType === checkProgressActionKey);
        }
        if (tutorResponse.activeItemDetail) {
            if (buttonType === 'hints' && tutorResponse.activeItemDetail.type === 'hint') return true;
            if (buttonType === 'steps' && tutorResponse.activeItemDetail.type === 'step') return true;
        ...
        if (buttonType === 'verify' && (tutorResponse.structuredResponse?.type === 'verify_steps' || tutorResponse.structuredResponse?.type === 'fix_errors_hint' || tutorResponse.structuredResponse?.type === 'fix_errors_step')) return true;
    ...
    const isExplanationButtonLoading = (uniqueKey) => {
        return isProcessingAudioForExplanation && activeExplanationKey === uniqueKey;
    };
    ...
    return (
        ...
        >
                ...
                <motion.div className="flex flex-row gap-2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    <motion.div variants={itemVariants}> <motion.div variants={buttonVariants} whileHover="hover" whileTap="tap">
                        <UIButton
                            ...
                            className={`flex items-center justify-center text-[16px] transition-all duration-300 text-primary border border-primary shadow-none ${isButtonSelected('hints') ? 'bg-tabs-background' : 'bg-white'} ${(isProcessingAudioForExplanation) ? 'opacity-50 cursor-not-allowed' : ''}`}
                            aria-pressed={isButtonSelected('hints')}
                        >
                            {isButtonLoading('next_hint') || isButtonLoading('next_3hint') ? <motion.div className="w-4 h-4 mr-1.5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : (buttonIcons.hints ? <Image src={isButtonSelected('hints') ? buttonIcons.hints.selected : buttonIcons.hints.unselected} alt="" width={12} height={12} aria-hidden="true" /> : <Lightbulb className="w-4 h-4 mr-1.5" aria-hidden="true" />)}
                            {getHintButtonLabel()}
                        </UIButton>
                    ...
                    <motion.div variants={itemVariants}> <motion.div variants={buttonVariants} whileHover="hover" whileTap="tap">
                        <UIButton
                            ...
                            disabled={(isLoading && currentButtonType === checkProgressActionKey) || !isWhiteboardDirty || isProcessingAudioForExplanation || (audioStatus.queueLength > 0 || audioStatus.isPaused)}
                            className={`flex items-center justify-center text-[16px] transition-all duration-300 text-primary border border-primary shadow-none ${isButtonSelected('verify') ? 'bg-tabs-background' : 'bg-white'} ${(!isWhiteboardDirty || isProcessingAudioForExplanation) ? 'opacity-50 cursor-not-allowed bg-gray-100 hover:bg-gray-100 border-gray-300 text-gray-500' : ''}`}
                            aria-pressed={isButtonSelected('verify')}
                        >
                            {isButtonLoading(checkProgressActionKey) ? <motion.div className="w-4 h-4 mr-1.5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : (buttonIcons.verify ? <Image src={isButtonSelected('verify') ? buttonIcons.verify.selected : buttonIcons.verify.unselected} alt="" width={12} height={12} aria-hidden="true" /> : <CheckCircle className="w-4 h-4 mr-1.5" aria-hidden="true" />)}
                            Check Progress
                        </UIButton>
                    </motion.div></motion.div>
                    <motion.div variants={itemVariants}> <motion.div variants={buttonVariants} whileHover="hover" whileTap="tap">
                        <UIButton
                            ...
                            disabled={(isLoading && (currentButtonType === 'next_step' || currentButtonType === 'next_3step')) || isProcessingAudioForExplanation || (audioStatus.queueLength > 0 || audioStatus.isPaused)}
                            className={`flex items-center justify-center text-[16px] transition-all duration-300 text-primary border border-primary shadow-none ${isButtonSelected('steps') ? 'bg-tabs-background' : 'bg-white'} ${(isProcessingAudioForExplanation) ? 'opacity-50 cursor-not-allowed' : ''}`}
                            aria-pressed={isButtonSelected('steps')}
                        >
                            {isButtonLoading('next_step') || isButtonLoading('next_3step') ? <motion.div className="w-4 h-4 mr-1.5 border-2 border-current border-t-transparent rounded-full animate-spin" /> : (buttonIcons.steps ? <Image src={isButtonSelected('steps') ? buttonIcons.steps.selected : buttonIcons.steps.unselected} alt="" width={12} height={12} aria-hidden="true" /> : <ArrowRight className="w-4 h-4 mr-1.5" aria-hidden="true" />)}
                            {getStepButtonLabel()}
                        </UIButton>
                    </motion.div></motion.div>
                </motion.div>
            ...
            <motion.div className="flex-grow overflow-y-auto space-y-3 mb-4" variants={itemVariants} role="log" aria-live="polite" aria-atomic="false" aria-relevant="additions text">
                                    ...
                                    {isUnlocked ? (
                                        ...
                                        <div className="px-2 mb-2">
                                            <div className="flex items-center space-x-2 align-middle">
                                                <Unlock className="w-5 h-5 text-gray-700 fill-gray-500" aria-hidden="true" />
                                                <UIButton
                                                    variant="link"
                ...
                <motion.div className="flex justify-between items-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} role="toolbar" aria-label="Final Actions">
                    <motion.div className="flex-grow mr-2" variants={buttonVariants} whileHover="hover" whileTap="tap">
                        <UIButton
                            ...
                            disabled={!isWhiteboardDirty || (isLoading && currentButtonType === 'verify') || (isAnyTextStreaming && currentButtonType !== null) || isProcessingAudioForExplanation || (audioStatus.queueLength > 0 || audioStatus.isPaused)}
                            className={`w-full text-sm transition-all duration-300 ${!isWhiteboardDirty || ((isAnyTextStreaming && currentButtonType !== null)) || isProcessingAudioForExplanation ? 'opacity-50 cursor-not-allowed' : ''} ${!isWhiteboardDirty ? 'bg-gray-100 hover:bg-gray-100 border-gray-300 text-gray-500' : 'bg-primary text-white hover:bg-primary/90'}`}
                        >
                            {isButtonLoading('verify') ? <><motion.div className="w-4 h-4 mr-2 border-2 border-white border-t-transparent rounded-full animate-spin" aria-hidden="true" /> Checking... </> : <> Check my final solution <ChevronRightIcon className="w-4 h-4 ml-1 inline-block" aria-hidden="true" /></>}
                        </UIButton>
                    </motion.div>
                    {[
                        ...
                        const isDisabled = (!hasAnyDisplayableContent && !tutorResponse.streamingText) || (isAnyTextStreaming && currentButtonType !== null) || isProcessingAudioForExplanation || (audioStatus.queueLength > 0 || audioStatus.isPaused);
                        return (
